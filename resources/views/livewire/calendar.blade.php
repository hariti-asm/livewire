...
<div class="flex relative">
    ...
    <div class="grid grid-cols-7 gap-1  px-4 ">
      @foreach ($days as $day)
        @if(!empty($day))
          <div class="aspect-square  flex items-center justify-center border border-1 h-10 w-12 rounded-sm  px-1 hover:border-[#00665C] hover:text-[#00665C]  @if ($day["day"] == $today && $day["month"] == $month) border-[#00665C] text-[#00665C] font-semibold @elseif ($day["day"] < $today &&  $day["month"] == $month || $day["month"] < $month )  line-through opacity-75 border-none disabled cursor-not-allowed @endif @if ( $day["month"] > $month) bg-white opacity-70 text-[#5a606c]
